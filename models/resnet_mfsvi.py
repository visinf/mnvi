...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1, prior_precision=1e0):
    ...
    return varprop.Conv2dMFS(in_planes, out_planes, kernel_size=3, stride=stride,
...
def conv1x1(in_planes, out_planes, stride=1, prior_precision=1e0):
    """1x1 convolution"""
    return varprop.Conv2dMFS(in_planes, out_planes, kernel_size=1, stride=stride, bias=False, prior_precision=prior_precision)
...
class ResNetMFSVI(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, groups=1, width_per_group=64,
                 ...
                 prior_precision=1e0):
        super(ResNetMFSVI, self).__init__()
        if norm_layer is None:
            norm_layer = varprop.BatchNorm2d
        ...
        self.conv1 = varprop.Conv2dMFS(3, self.inplanes, kernel_size=3, stride=1, padding=1,
        ...
        self.fc = varprop.LinearMFS(512 * block.expansion, num_classes, prior_precision=self._prior_precision)
...
class ResNet18MFSVI(nn.Module):
    def __init__(self, num_classes=10, min_variance=1e-5, prior_precision=1e0, kl_div_weight=0.0, **kwargs):
        super(ResNet18MFSVI, self).__init__()
        self.resnet = ResNetMFSVI(BasicBlock, [2, 2, 2, 2], num_classes=num_classes,
